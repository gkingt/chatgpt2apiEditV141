...
class AccountService:
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        normalized["image_success_count"] = int(normalized.get("image_success_count") or 0)
        normalized["image_failure_count"] = int(normalized.get("image_failure_count") or 0)
    ...
    @staticmethod
    def _parse_account_time(value: object) -> float:
        raw = str(value or "").strip()
        if not raw:
            return 0.0
    ...
    def _image_candidate_score(self, token: str) -> tuple[float, str]:
        account = self._accounts.get(token) or {}
        inflight = int(self._image_inflight.get(token, 0))
        score = -200.0 * inflight
        if bool(account.get("image_quota_unknown")):
            score += 30.0
        else:
            score += min(80.0, float(max(0, int(account.get("quota") or 0))))

        plan = self._normalize_account_type(account.get("type")) or "free"
        plan_bonus = {
            "Plus": 18.0,
            "Pro": 28.0,
            "ProLite": 14.0,
        ...
        score += plan_bonus.get(plan, 0.0)

        image_success = int(account.get("image_success_count") or 0)
        image_failure = int(account.get("image_failure_count") or 0)
        consecutive_failures = int(account.get("image_consecutive_failures") or 0)
        score += min(50.0, image_success * 2.0)
        score -= min(80.0, image_failure * 3.0)
        ...
        if last_failure and now - last_failure < 15 * 60:
            score -= 70.0
        last_success = self._parse_account_time(account.get("last_image_success_at"))
        if last_success and now - last_success < 24 * 60 * 60:
            score += 20.0

        last_used = self._parse_account_time(account.get("last_used_at"))
        # Tiny tie-breaker: prefer accounts that have rested a bit longer.
        score += min(10.0, max(0.0, now - last_used) / 3600.0) if last_used else 10.0
        return (score, token)
    ...
    def _list_available_candidate_tokens(
        ...
        return sorted(tokens, key=self._image_candidate_score, reverse=True)
    ...
    def get_available_access_token(
        ...
        """
        initial_candidate_count = len(self._list_ready_candidate_tokens(
            plan_type=plan_type,
    ...
    def mark_image_result(self, access_token: str, success: bool, *, release_slot: bool = True) -> dict | None:
        ...
        with self._lock:
            ...
            if success:
                ...
                next_item["image_success_count"] = int(next_item.get("image_success_count") or 0) + 1
                next_item["image_consecutive_failures"] = 0
                next_item["last_image_success_at"] = now
