...
export type SettingsConfig = {
  ...
  image_default_model_slug?: string;
  image_fallback_model_slug?: string;
  image_account_concurrency?: number | string;
  image_redundant_generation_enabled?: boolean;
  image_redundant_copies?: number | string;
  image_redundant_max_attempts?: number | string;
  image_parallel_generation?: boolean;
  image_settle_enabled?: boolean;
  image_check_before_hit_enabled?: boolean;
  image_chat_panel_enabled?: boolean;
...
export async function resumeImagePoll(taskId: string, extraTimeoutSecs = 30, conversationId?: string) {
...
export async function stopImageTask(taskId: string) {
  return httpRequest<ImageTask>(`/api/image-tasks/${encodeURIComponent(taskId)}/stop`, {
    method: "POST",
    body: {},
  });
}
...
export async function fetchPromptLibrary() {
  return httpRequest<PromptLibraryResponse>("/api/prompt-library");
}

export async function createPromptLibraryItem(item: { name: string; type: string; content: string; note?: string }) {
  return httpRequest<PromptLibraryResponse>("/api/prompt-library", {
    method: "POST",
    body: item,
  });
...
export async function updatePromptLibraryItem(id: string, item: { name: string; type: string; content: string; note?: string }) {
  return httpRequest<PromptLibraryResponse>(`/api/prompt-library/${encodeURIComponent(id)}`, {
    method: "POST",
    body: item,
  });
}

export async function deletePromptLibraryItem(id: string) {
  return httpRequest<PromptLibraryResponse>(`/api/prompt-library/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
...
export async function fetchMaterialLibrary() {
  return httpRequest<MaterialLibraryResponse>("/api/material-library");
}

export async function uploadMaterialLibraryItem(file: File, item: { name: string; type: string; note?: string }) {
  const formData = new FormData();
  formData.append("file", file);
  formData.append("name", item.name);
  formData.append("type", item.type);
  formData.append("note", item.note || "");
  return httpRequest<MaterialLibraryResponse>("/api/material-library/upload", {
    method: "POST",
    body: formData,
  });
}

export async function createMaterialFromBase64(item: {
  name: string;
  type: string;
  note?: string;
  b64_json: string;
  ...
  return httpRequest<MaterialLibraryResponse>("/api/material-library/from-base64", {
    method: "POST",
    body: item,
  });
}

export async function updateMaterialLibraryItem(id: string, item: { name?: string; type?: string; note?: string }) {
  return httpRequest<MaterialLibraryResponse>(`/api/material-library/${encodeURIComponent(id)}`, {
    method: "POST",
    body: item,
  });
}
...
export async function startRegister(config: Partial<RegisterConfig>) {
  return httpRequest<{ register: RegisterConfig }>("/api/register/start", { method: "POST", body: config });
}
