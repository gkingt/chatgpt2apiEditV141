...
import { ArrowDown, History, ImageIcon, LoaderCircle, MessageSquareText, Plus, Trash2 } from "lucide-react";
...
const USER_STOPPED_IMAGE_ERROR = "已手动停止生成";
...
function activeConversationQueueKey(conversationId: string, turnId: string) {
...
function taskDataToStoredImage(image: StoredImage, task: ImageTask): StoredImage {
  if (task.status === "success") {
    ...
    if (!first?.b64_json && !first?.url) {
      return {
        ...image,
        taskId: task.id,
        upstreamConversationId: task.conversation_id || image.upstreamConversationId,
        status: "error",
  ...
  const shouldSetStartTime = !image.startTime;
  const startTime = shouldSetStartTime ? Date.now() : image.startTime;
  // elapsedSecs 仅使用后端返回的值，确保计时从 image_stream_resolve_start 开始
  const elapsedSecs = typeof task.elapsed_secs === "number" ? task.elapsed_secs : undefined;
...
function getMaterialFileName(item: MaterialLibraryItem, fallbackIndex: number) {
  const original = item.filename || item.name;
  if (/\.[a-z0-9]+$/i.test(original)) {
    return original;
  }
  const extension =
    item.mime_type === "image/jpeg"
      ...
      : item.mime_type === "image/webp"
        ...
        : item.mime_type === "image/gif"
          ? "gif"
          : "png";
  return `${item.name || `material-${fallbackIndex + 1}`}.${extension}`;
}

function taskDataToStoredImages(image: StoredImage, task: ImageTask): StoredImage[] {
  if (task.status !== "success") {
    return [taskDataToStoredImage(image, task)];
  }
  const items = (task.data || []).filter((item) => item.b64_json || item.url);
  if (items.length <= 1) {
    return [taskDataToStoredImage(image, task)];
  }
  return items.map((item, index) => ({
    ...image,
    id: index === 0 ? image.id : `${image.id}-redundant-${index}`,
    taskId: index === 0 ? task.id : undefined,
    upstreamConversationId: task.conversation_id || image.upstreamConversationId,
...
function shouldRefreshConversationUpdatedAt(previousTurns: ImageTurn[], nextTurns: ImageTurn[]) {
  return nextTurns.some((nextTurn, index) => {
    const previousTurn = previousTurns[index];
    if (!previousTurn) {
      return false;
    }
    const wasActive = previousTurn.status === "queued" || previousTurn.status === "generating";
    const isTerminal = nextTurn.status === "success" || nextTurn.status === "error";
...
async function syncConversationImageTasks(items: ImageConversation[]) {
  ...
  const normalized = items.map((conversation) => {
    const turns = conversation.turns.map((turn) => {
      ...
      const images = turn.images.flatMap((image) => {
        if (!image.taskId) {
          return [image];
        }
        ...
        const nextImages = taskDataToStoredImages(image, task);
        if (nextImages.length !== 1 || nextImages[0] !== image) {
          turnChanged = true;
        }
        return nextImages;
      });
    ...
    return {
      ...conversation,
      turns,
      updatedAt: shouldRefreshConversationUpdatedAt(conversation.turns, turns)
...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const showImageChatPanel = config?.image_chat_panel_enabled !== false;
  ...
  useEffect(() => {
    if (!showImageChatPanel && mobilePanel === "chat") {
      setMobilePanel("image");
    }
  }, [mobilePanel, showImageChatPanel]);

  useEffect(() => {
    if (typeof window === "undefined") {
      return;
    }
    setSelectedPromptIds(loadStoredIdList(IMAGE_SELECTED_PROMPTS_STORAGE_KEY));
    setSelectedMaterialIds(loadStoredIdList(IMAGE_SELECTED_MATERIALS_STORAGE_KEY));
    setChatConfig({
  ...
  const loadLibraries = useCallback(async () => {
    try {
      const [promptData, materialData] = await Promise.all([
        fetchPromptLibrary(),
        fetchMaterialLibrary(),
      ]);
      setPromptLibraryItems(promptData.items);
      setMaterialLibraryItems(materialData.items);
      setSelectedPromptIds((current) => {
        const validIds = new Set(promptData.items.map((item) => item.id));
        const next = current.filter((id) => validIds.has(id));
        saveStoredIdList(IMAGE_SELECTED_PROMPTS_STORAGE_KEY, next);
        return next;
      });
      setSelectedMaterialIds((current) => {
        const validIds = new Set(materialData.items.map((item) => item.id));
        const next = current.filter((id) => validIds.has(id));
        saveStoredIdList(IMAGE_SELECTED_MATERIALS_STORAGE_KEY, next);
        return next;
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "读取提示词/素材库失败";
      toast.error(message);
    }
  }, []);
  ...
  const handleAddImageToMaterials = useCallback(async (image: StoredImage, index: number) => {
    try {
      ...
      const response = image.b64_json
        ...
        : image.url
          ? await uploadMaterialLibraryItem(
              await fetchImageAsFile(image.url, filename),
              {
                name,
                type: "生成结果",
                note,
  ...
  const runConversationQueue = useCallback(
    async (conversationId: string, preferredTurnId?: string) => {
      ...
      const activeTurn = preferredTurnId
        ? snapshot?.turns.find(
            (turn) =>
              turn.id === preferredTurnId &&
      ...
      const queueKey = activeConversationQueueKey(conversationId, activeTurn.id);
      if (activeConversationQueueIds.has(queueKey)) {
        return;
      }

      activeConversationQueueIds.add(queueKey);
        ...
        await updateConversation(conversationId, (current) => {
          ...
          const turns = conversation.turns.map((turn) => {
            ...
            const images = turn.images.flatMap((image) => {
              ...
              if (image.status === "error" && image.error === USER_STOPPED_IMAGE_ERROR) {
                return [image];
              }
              return task ? taskDataToStoredImages({ ...image, taskId }, task) : [image];
            });
            const derived = deriveTurnStatus({ ...turn, images });
            return {
              ...turn,
      ...
      try {
        ...
        activeConversationQueueIds.delete(queueKey);
        for (const conversation of conversationsRef.current) {
          for (const turn of conversation.turns) {
            if (
              !activeConversationQueueIds.has(activeConversationQueueKey(conversation.id, turn.id)) &&
              (turn.status === "queued" || turn.status === "generating") &&
              turn.images.some((image) => image.status === "loading")
            ) {
  ...
  const handleTimeoutRetryContinue = useCallback(async (conversationId: string, turnId: string, taskId: string) => {
    try {
      const localConversation = conversationsRef.current.find((item) => item.id === conversationId);
      const localTurn = localConversation?.turns.find((turn) => turn.id === turnId);
      const localImage = localTurn?.images.find((image) => (image.taskId || image.id) === taskId);
      const upstreamConversationId = localImage?.upstreamConversationId;
      await resumeImagePoll(taskId, imageTimeoutRetrySecs, upstreamConversationId);
            ...
            return {
              ...
              images: turn.images.map((image) =>
                image.taskId === taskId
                  ? {
                      ...image,
                      upstreamConversationId: image.upstreamConversationId || upstreamConversationId,
                      status: "loading" as const,
                      error: undefined,
      ...
      // 清除重试状态
      setTimeoutRetry((current) => current?.taskId === taskId ? null : current);
      void runConversationQueue(conversationId, turnId);
  ...
  }, [runConversationQueue, updateConversation, imageTimeoutRetrySecs]);

  const handleStopImage = useCallback(
    async (conversationId: string, turnId: string, taskId: string) => {
      try {
        await stopImageTask(taskId);
      } catch {
        // The task can be missing briefly if the user stops during submit; keep the UI responsive.
      }
      await updateConversation(conversationId, (current) => {
        const conversation = current ?? conversationsRef.current.find((c) => c.id === conversationId);
        if (!conversation) return current!;
        const turns = conversation.turns.map((turn) => {
          ...
          const images = turn.images.map((image) => {
            ...
            const startedAt = typeof image.startTime === "number" ? image.startTime : Date.now();
            return {
              ...image,
              status: "error" as const,
              taskStatus: undefined,
              progress: undefined,
              error: USER_STOPPED_IMAGE_ERROR,
  ...
  const handleSubmit = async () => {
    ...
    const selectedPrompts = promptLibraryItems.filter((item) => selectedPromptIds.includes(item.id));
    const selectedPromptText = selectedPrompts
      .map((item) => item.content.trim())
      .filter(Boolean)
      .join("\n\n");
    const finalPrompt = selectedPromptText ? `${selectedPromptText}\n\n${prompt}` : prompt;

    const selectedMaterials = materialLibraryItems.filter((item) => selectedMaterialIds.includes(item.id));
    let materialReferenceFiles: File[] = [];
    let materialReferenceImages: StoredReferenceImage[] = [];
    if (selectedMaterials.length > 0) {
      try {
        const materialReferences = await Promise.all(
          selectedMaterials.map(async (item, index) => {
            const file = await fetchImageAsFile(item.url, getMaterialFileName(item, index));
            return {
              file,
              referenceImage: {
                name: item.name || file.name,
                type: file.type || item.mime_type || "image/png",
                dataUrl: await readFileAsDataUrl(file),
              },
            };
          }),
        );
        materialReferenceFiles = materialReferences.map((item) => item.file);
        materialReferenceImages = materialReferences.map((item) => item.referenceImage);
      } catch (error) {
        const message = error instanceof Error ? error.message : "读取选中素材失败";
        toast.error(message);
        return;
      }
    }

    const combinedReferenceImageFiles = [...referenceImageFiles, ...materialReferenceFiles];
    const combinedReferenceImages = [...referenceImages, ...materialReferenceImages];
    const effectiveImageMode: ImageConversationMode = combinedReferenceImageFiles.length > 0 ? "edit" : "generate";
  ...
  return (
    <>
      <section
        className="mx-auto grid h-full min-h-0 w-full max-w-[1760px] grid-cols-1 gap-2 overflow-x-hidden overflow-y-auto px-0 pb-[calc(env(safe-area-inset-bottom)+4.75rem)] transition-[grid-template-columns] duration-200 sm:gap-3 sm:px-3 sm:pb-[calc(env(safe-area-inset-bottom)+5rem)] lg:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[var(--image-layout-cols)] xl:overflow-hidden xl:pb-6"
        style={{
          "--image-layout-cols": !showImageChatPanel
            ? "240px minmax(0,1fr)"
            : isChatCollapsed
              ? "240px minmax(0,1fr) 52px"
              : "240px minmax(0,1fr) 430px",
        } as CSSProperties}
      >
          ...
          <div className="relative min-h-0 flex-1">
            ...
            >
              <ImageResults
                ...
                onAddImageToMaterials={handleAddImageToMaterials}
                onTimeoutRetryContinue={handleTimeoutRetryContinue}
                onStopImage={handleStopImage}
        ...
        {showImageChatPanel ? (
          <ImageChatPanel
            config={chatConfig}
            collapsed={mobilePanel === "chat" ? false : isChatCollapsed}
      ...
      {showImageChatPanel ? (
      <div className="fixed bottom-[calc(env(safe-area-inset-bottom)+0.75rem)] left-3 z-50 inline-flex rounded-2xl border border-stone-200 bg-white/95 p-1 shadow-lg shadow-stone-200/60 backdrop-blur xl:hidden dark:border-white/10 dark:bg-stone-900/95 dark:shadow-black/30">
        <button
          type="button"
          className={cn(
            "inline-flex h-10 items-center gap-1.5 rounded-xl px-3 text-xs font-medium transition",
            mobilePanel === "image" ? "bg-stone-950 text-white" : "text-stone-600 hover:bg-stone-100",
          )}
        ...
        <button
          ...
          className={cn(
            "inline-flex h-10 items-center gap-1.5 rounded-xl px-3 text-xs font-medium transition",
            mobilePanel === "chat" ? "bg-stone-950 text-white" : "text-stone-600 hover:bg-stone-100",
          )}
